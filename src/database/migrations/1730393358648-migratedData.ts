...
export class MigratedData1730393358648 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        try {
            ...
            await queryRunner.manager.transaction(async (transactionalEntityManager: EntityManager) => {
                const businessUnitId = uuidv4();
                ...
                // Inserción en la tabla users_v1
                await transactionalEntityManager.query(
                    `INSERT INTO users_v1 (id, name, credential_id, rol_id, created_at, updated_at) VALUES ?`,
                    [userInserts.map(user => [
                        user.id,
                        user.name,
                        user.credential_id,
                        user.rol_id,
                ...
                const bucketMap = new Map(
                    (await transactionalEntityManager.query(`SELECT project_id, id as bucket_id FROM bucket_v1`))
                ...
                for (const folder of allFolders) {
                    ...
                    if (parent) parentUpdates.push({ id, parent });
                }
                ...
                for (const { id, parent } of parentUpdates) {
                    const parentExists = await transactionalEntityManager.query(
                        `SELECT id FROM folder_v1 WHERE id = '${parent}'`
                    );
                ...
                for (const att of allAttachments) {
                    ...
                    const bucket_id = (await transactionalEntityManager.query(
                        `SELECT bucket_id FROM folder_v1 WHERE id = '${folder_id}'`
                    ))[0]?.bucket_id;
                ...
                await transactionalEntityManager.query(
                    `INSERT INTO attachment_v1 (id, file_name, file_path, file_size, file_type, bucket_id, folder_id, created_at, updated_at) VALUES ?`,
                    [attachmentInserts.map(att => [
                        att.id,
                        att.file_name,
                        att.file_path,
                        att.file_size,
